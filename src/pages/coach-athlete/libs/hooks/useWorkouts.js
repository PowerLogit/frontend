...
import { getWorkoutsCoachAthleteService } from '../services/workoutCoach.service'

const useWorkouts = (idAthlete, filters) => {
...
const loadWorkouts = async (idAthlete, setters, filters, signal) => {
    setters.setLoading()

    const { workout, count, error, isAborted } =
        await getWorkoutsCoachAthleteService(idAthlete, filters, signal)
